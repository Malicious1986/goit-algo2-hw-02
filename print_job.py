...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    jobs = [(i, PrintJob(**job)) for i, job in enumerate(print_jobs)]
    printer = PrinterConstraints(**constraints)

    sorted_jobs = sorted(jobs, key=lambda x: (x[1].priority, x[0]))
    ...
    while sorted_jobs:
        ...
        for idx, job in sorted_jobs:
            ...
            if can_add:
                current_batch.append((idx, job))
                current_volume += job.volume
                current_max_time = max(current_max_time, job.print_time)
                jobs_to_remove.append((idx, job))

        for item in jobs_to_remove:
            sorted_jobs.remove(item)

        if current_batch:
            print_order.extend([job.id for _, job in current_batch])
